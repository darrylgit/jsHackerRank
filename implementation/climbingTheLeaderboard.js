...
function climbingLeaderboard(scores, alice) {
  ...
  let scoresArray = Array.from(scoresSet);

  let leaderboardRankings = [];

  alice.forEach(aliceScore => {
    if (scoresArray.includes(aliceScore)) {
      let indexOfScore = scoresArray.indexOf(aliceScore) + 1;
      scoresArray = scoresArray.slice(0, indexOfScore + 1);
      leaderboardRankings.push(indexOfScore);
    } else {
      let scoresWithAlice = [...scoresArray, aliceScore].sort((a, b) => b - a);

      leaderboardRankings.push(scoresWithAlice.indexOf(aliceScore) + 1);
    }
  });
...
console.log(climbingLeaderboard([2, 2, 3, 4, 5, 7, 5, 5, 6], [1, 7]));
